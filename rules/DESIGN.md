...
| 样式隔离 | 组件样式在组件内 · 页面样式在页面内 · 全局 Token 在 `rules/WebVariable/` |
...
```
rules/WebVariable/     # 设计 Token 与全局重置（唯一维护处，业务项目不重复定义）
...
| 文件 | 职责 |
|------|------|
| `rules/WebVariable/ThemeVariable.css` | **主题色与文本色**（唯一维护处） |
| `rules/WebVariable/SystemVariable.css` | 间距、布局、字号、图标尺寸、边框宽度、复合边框/阴影 |
| `rules/WebVariable/ProjectReset.css` | 全局重置、`html`/`body` 基础样式 |
| `rules/WebVariable/Animation.css` | **全局动画工具类**：入场、强调、加载 spinner、骨架屏、滚动显现等；业务优先复用类名，**禁止**在组件内散写未登记的 `@keyframes` |
...
当前默认启用 **通用界面设计规范**（`ThemeVariable.css` + `SystemVariable.css` + `ProjectReset.css` + `Animation.css` 核心套件）。开发者若需不同风格，可在 `rules/WebVariable/` 或 `rules/Extensions/` **追加**扩展 CSS 并在入口引入，**不修改核心文件**。详见 `AGENTS.md` §3.4。

### 2.4 核心变量索引

色值以 **`rules/WebVariable/ThemeVariable.css`** 为唯一源码；间距/字号/复合边框以 **`SystemVariable.css`** 为准。视觉对照在浏览器打开 **`rules/PreView/LightDesignSpec.html`** · **`DarkDesignSpec.html`**（勿将预览 HTML 全文载入 AI 上下文）。
...
4. 擅自修改 `rules/WebVariable/` 核心文件（扩展套件除外且须开发者明确要求）
...
*最后同步：`rules/WebVariable/`（含 Animation.css） · `AGENTS.md` · `rules/PreView/`*
