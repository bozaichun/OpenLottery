...
**仓库角色**：**project-coding-rules-file** 用于制定并分发 **AI 可读的编程规范**；本文件约束 **工程实现与代码质量**。**AI 协作时的过程与变更粒度**见 **`AGENTS.md` §2**；**视觉与设计 Token 的语义**见 **`rules/DESIGN.md`**；二者与本文件同时适用时以 **业务项目已定稿的设计系统** 为准。
...
**通用质量基线**见 `AGENTS.md` §6（编码风格 / 提交门禁 / 分层 / 接口 / 质量 / 技术债 / 可维护性）。
...
- **Token**：颜色、间距、字号引用 **项目内设计变量**（见 `rules/DESIGN.md` 与 **`rules/WebVariable/`**）。禁止在业务组件内复制**整套**色板常量；若缺 Token，应在 **全局主题或变量文件** 增补后再引用。改 Token 源码时优先读对应 CSS，无需同时加载本文件全文。
...
- **移动 / 平板**：首屏与交互目标见 **`rules/DESIGN.md`**（触控尺寸、断点、安全区）；**勿**假设仅桌面键鼠。
...
**通用基线**见 `AGENTS.md` §6（编码风格与提交强制校验、目录分层、接口规范、质量、依赖与技术债、可维护性）。以下为本栈落点：
...
- [ ] 已遵守 AGENTS.md §6：lint/format 门禁、分层、接口契约与文档同步
...
## 与 `rules/DESIGN.md` 的分工

| 主题 | `CodingSpec.md`（本文件） | `rules/DESIGN.md` |
|------|---------------------------|-------------|
| AI 写码心智（先问再做、最小 diff） | 见 **`AGENTS.md` §2** | 见 **`AGENTS.md` §2** |
| Token 命名与色板角色 | 引用方式、禁止魔法数 | **变量索引、用法、UI 模式**（色值见 `WebVariable/`） |
| 按钮/表单长成什么样 | 组件拆分、状态、可访问属性 | **颜色、间距、字号、状态色** |
| 响应式 | 目录与性能 | **断点原则、触控目标** |
| 质量门禁 / 技术债 | **`AGENTS.md` §6** + 本章 §12 | — |
